...
#    --optimize=<sequest|mascot|tandem>  --input_files='file1,file2'
#
#
...
class IonOptimizer
  ...
  def self.optimize(ion_array = [], algo = 'mascot')
    ...
    when 'mascot'
      ...
      ion_array.sort!{ |x,y| y.intensity <=> x.intensity} #sort ions descreasing intensity and take top 200
      return ion_array[0..199].sort { |x,y| x.mz <=> y.mz}
...
class MGFPrinter
  ...
  def self.print(ions, s)
    ...
    mgfout +=  "BEGIN IONS\n"
    mgfout +=  "TITLE=#{s.id}\n"
    mgfout +=  "RTINSECONDS=#{"%0.5f" % s.retention_time}\n"
    mgfout +=  "PEPMASS=#{"%0.5f" % s.precursor_mass} #{"%0.2f" % s.precursor_intensity}\n"

    0.upto(ions.length-1) do |i|
      mgfout +=  "#{sprintf('%0.7f', ions[i].mz)} #{sprintf('%0.2f', ions[i].intensity)}\n"
    end
...
OptionParser.new do |opts|
  opts.banner = "Usage: spectra_preprocessor.rb --optimize=<sequest|mascot|tandem> --input_files=<file1.mzML,file2.mzML>"
